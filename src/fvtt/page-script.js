...
function updateHP(name, current, total, temp) {
    ...
    // dnd5e keeps Max HP modifiers (Aid, Heroes' Feast, a specter's life drain) in
    // attributes.hp.tempmax, not in attributes.hp.max, and shows max + tempmax as the
    // effective maximum. Writing D&D Beyond's total straight into max overwrote the
    // actor's own maximum on every sync, so the sheet's Temp Max box never moved (#1158).
    //
    // Write the modifier as tempmax = total - max and leave max alone. An actor whose max
    // is still 0 has never been filled in, so give it one from the total on the first sync.
