...
Every push to `master` publishes a new build. Chrome picks it up within a few hours,
...
```bash
git clone https://github.com/Jombolio/Beyond20.git
cd Beyond20
npm install
npm run build
```
...
```
https://raw.githubusercontent.com/Jombolio/Beyond20/master/FVTT-module/beyond20/module.json
```
...
`.github/workflows/chrome-dev.yml` runs on every push to `master`. It builds the
